...
import {
  handleEditClusterOpen,
  handleDeleteClusterOpen,
  handleFavoriteChange,
} from "../../../utils/clustersHandler";
...
const ClusterCard = ({ clusterObj }) => {
  ...
  useEffect(() => {
    if (clusterObj.alertTopics.length < 1) setRedExist("green");
      ...
      clusterObj.alertTopics.reduce(
        (includeRed, alertTopic) => includeRed || alertTopic.color === "red",
        false
      )
    )
      setRedExist("red");
  ...
  return (
    ...
    <Card boxShadow="lg">
      <CardHeader>
        <Flex>
          {/* Title */}
          <Heading w="75%" fontSize={22} m={"auto"}>
            {clusterObj.name}
          </Heading>
          ...
          <IconButton
            _hover={""}
            isRound={true}
            aria-label="add favorite cluster"
            variant="ghost"
            icon={
              clusterObj.favorite ? (
                <Icon as={RxStarFilled} color="yellow.300" boxSize={6} />
              ) : (
                <Icon as={RxStar} boxSize={5} />
              )
            }
            onClick={() => {
              handleFavoriteChange(toast, clusterObj._id);
            }}
          />
            ...
            <Flex w="full" justifyContent="center" alignItems="center">
              <Box maxH={12} overflow={"hidden"}>
                {clusterObj.alertTopics.length > 0
                  ? clusterObj.alertTopics
                      .map((alertTopic, index) => {
                        return (
                          <Badge
                            key={"alertBadge" + index}
                            colorScheme={alertTopic.color}
                            mx={0.5}
                          >
                            {alertTopic.text}
                          </Badge>
      ...
      <CardFooter>
        <Flex width="full">
          ...
          <IconButton
            ...
            variant="ghost"
            onClick={() => handleEditClusterOpen(clusterObj)}
